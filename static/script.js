...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Handle ?checkout=1 arriving from product Buy Now (used after login)
        ...
        menuGrid.innerHTML += `
            <div class="food-card" onclick="window.location.href='/product/${food.id}'" style="cursor: pointer;">
                <div class="food-img-container">
                    <img src="/${food.image}" onerror="this.src='https://via.placeholder.com/300x200?text=No+Image'">
                </div>
...
function applyFilters() {
    ...
    if (query) filtered = filtered.filter(f => f.name.toLowerCase().includes(query) || (f.description && f.description.toLowerCase().includes(query)));
...
window.showProductDetails = function (id) {
    ...
    imgObj.src = `/${food.image}`;
    imgObj.onerror = function () { this.src = 'https://via.placeholder.com/600x400?text=No+Image'; };

    document.getElementById('pdTitle').innerText = food.name;
    document.getElementById('pdPrice').innerText = `₹${food.price}`;
    document.getElementById('pdDescription').innerText = food.description || 'No description available for this delicious item.';
...
function toggleAuthMode() {
    ...
    document.getElementById('authSubtitle').innerText = isLoginMode ? 'Sign in to unlock amazing features' : 'Join us to get the best food delivered';
...
async function handleAuth(e) {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            document.querySelector('#accountLink .nav-text').innerText = 'My Account';
            ...
            showView('menu-view');
            // If user arrived via "Buy Now" from product page, trigger checkout
...
window.logout = function () {
    currentUser = null;
    document.querySelector('#accountLink .nav-text').innerText = 'Sign In';
    showView('menu-view');
...
function updateCartUI() {
    ...
    if (cart.length === 0) {
        ...
        if (document.getElementById('qrPayTotal')) document.getElementById('qrPayTotal').innerText = '0';
        return;
    }
...
async function handlePayment(e) {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            if (currentUser) document.getElementById('payName').value = currentUser.name;
        } else {
...
window.trackOrder = async function () {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            document.getElementById('trackStatusText').innerText = `Status: ${order.status}`;
            document.getElementById('trackOrderTotal').innerText = `Total Amount: ₹${order.total}`;
            ...
            if (order.status === 'Pending') {
                ...
                document.getElementById('trackStatusText').innerText = `Status: Cancelled`;
